...
pub struct Payer {
    ...
    /// Email associated with the payer. This value will only return a response when `status` is `"approved"`, `refunded` or `charged_back`.
    pub email: String,
    /// Personal identification of the user. For example, in Brazil, we have CPF for individuals and CNPJ for companies. Other possible identification codes include CURP (Mexico) and CUIL (Argentina). This object will only return a response when `status` is `"approved"`, `"refunded"` or `"charged_back"`.
